...
//
// Returns both the connection target and a flag indicating whether the
// docker-compose path was taken, so `afterAll` only tears down docker
// services that this test actually started. Unconditionally calling
// `dockerCompose.down()` would remove every service in the shared
// `bun-test-services` compose project — e.g. the containers another
// concurrently-running suite depends on.
async function resolvePostgres(): Promise<{ host: string; port: number; usedDocker: boolean } | null> {
  try {
    const info = await dockerCompose.ensure("postgres_plain");
    return { host: info.host, port: info.ports[5432], usedDocker: true };
  } catch {}
  ...
  try {
    ...
    return { host: "127.0.0.1", port: 5432, usedDocker: false };
  } catch {}
...
describe("issue #29010 — Date parameters serialize as ISO 8601", async () => {
  ...
  afterAll(async () => {
    if (target.usedDocker && !process.env.BUN_KEEP_DOCKER) {
      try {
